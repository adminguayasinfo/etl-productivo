"""
Pipeline operational para fertilizantes.
...
class FertilizantesOperationalRefactorizedPipeline:
    """Pipeline para procesar datos de fertilizantes de staging a operational."""
    ...
    def run(self) -> Dict[str, Any]:
        ...
        """
        logger.info("=== Iniciando Pipeline Operational Refactorizado Fertilizantes ===")
        ...
        try:
            ...
            logger.info("\n=== Pipeline Operational Refactorizado Fertilizantes Completado ===")
    ...
    def _get_pending_count(self, session: Session) -> int:
        ...
        return session.query(StgFertilizante).filter(
            StgFertilizante.processed == False
        ).count()
    
    def _read_staging_batch(self, session: Session) -> List[StgFertilizante]:
        """Lee un lote de registros desde staging."""
        return session.query(StgFertilizante).filter(
            StgFertilizante.processed == False
        ).order_by(StgFertilizante.id).limit(self.batch_size).all()
    
    def _process_batch(self, session: Session, staging_records: List[StgFertilizante]) -> Dict[str, int]:
    ...
    def _process_single_record(self, session: Session, record: StgFertilizante):
        ...
        # 5. Crear beneficio de fertilizantes
        beneficio = BeneficioFertilizantes.create_from_staging(
        ...
        self.stats['entities_created']['beneficios_fertilizantes'] += 1
    
    def _get_or_create_direccion(self, session: Session, record: StgFertilizante) -> Direccion:
    ...
    def _get_or_create_beneficiario(self, session: Session, record: StgFertilizante, 
