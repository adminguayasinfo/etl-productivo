"""
Pipeline operational para semillas.
...
class SemillasOperationalRefactorizedPipeline:
    """Pipeline para procesar datos de semillas de staging a operational."""
    ...
    def run(self) -> Dict[str, Any]:
        ...
        """
        logger.info("=== Iniciando Pipeline Operational Semillas ===")
        ...
        try:
            ...
            logger.info("\n=== Pipeline Operational Semillas Completado ===")
    ...
    def _read_staging_batch(self, session: Session) -> List[StgSemilla]:
        """Lee un lote de registros desde staging."""
        return session.query(StgSemilla).filter(
            StgSemilla.processed == False
        ).order_by(StgSemilla.id).limit(self.batch_size).all()
    
    def _process_batch(self, session: Session, staging_records: List[StgSemilla]) -> Dict[str, int]:
    ...
    def _process_single_record(self, session: Session, record: StgSemilla):
        ...
        asociacion = None
        if record.organizacion and record.organizacion.strip():
            asociacion = self._get_or_create_asociacion(session, record.organizacion.strip())
        ...
        # 5. Crear beneficio de semillas
        beneficio = BeneficioSemillas.create_from_staging(
        ...
        self.stats['entities_created']['beneficios_semillas'] += 1
    
    def _get_or_create_direccion(self, session: Session, record: StgSemilla) -> Direccion:
        ...
        direccion = Direccion.get_or_create_by_location(
            ...
            recinto=(record.localidad or '').strip(),  # En semillas se llama 'localidad'
    ...
    def _get_or_create_beneficiario(self, session: Session, record: StgSemilla, 
