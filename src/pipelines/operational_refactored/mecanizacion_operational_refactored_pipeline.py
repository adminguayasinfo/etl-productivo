"""
Pipeline operational para mecanización.
Carga datos desde stg_mecanizacion a la estructura operational usando BeneficioMecanizacion.
"""
...
class MecanizacionOperationalRefactorizedPipeline:
    """Pipeline para procesar datos de mecanización de staging a operational."""
    ...
    def run(self) -> Dict[str, Any]:
        ...
        """
        logger.info("=== Iniciando Pipeline Operational Mecanización ===")
        ...
        try:
            ...
            logger.info("\n=== Pipeline Operational Mecanización Completado ===")
    ...
    def _get_pending_count(self, session: Session) -> int:
        ...
        return session.query(StgMecanizacion).filter(
            StgMecanizacion.processed == False
        ).count()
    
    def _read_staging_batch(self, session: Session) -> List[StgMecanizacion]:
        """Lee un lote de registros desde staging."""
        return session.query(StgMecanizacion).filter(
            StgMecanizacion.processed == False
        ).order_by(StgMecanizacion.id).limit(self.batch_size).all()
    
    def _process_batch(self, session: Session, staging_records: List[StgMecanizacion]) -> Dict[str, int]:
    ...
    def _process_single_record(self, session: Session, record: StgMecanizacion):
        ...
        asociacion = None
        if record.agrupacion and record.agrupacion.strip():
            asociacion = self._get_or_create_asociacion(session, record.agrupacion.strip())
        
        # 3. Crear/obtener tipo de cultivo
        tipo_cultivo = self._get_or_create_tipo_cultivo(session, record.cultivo)
        
        # 4. Crear/obtener beneficiario
        beneficiario = self._get_or_create_beneficiario(session, record, direccion, asociacion)
        
        # 5. Crear beneficio de mecanización
        beneficio = BeneficioMecanizacion.create_from_staging(
        ...
        self.stats['entities_created']['beneficios_mecanizacion'] += 1
    
    def _get_or_create_direccion(self, session: Session, record: StgMecanizacion) -> Direccion:
        """Crea o obtiene una dirección."""
        # Convertir coordenadas string a decimal si es posible
        coord_x = None
        coord_y = None
        
        try:
            if record.coord_x and record.coord_x.strip():
                coord_x = float(record.coord_x.strip())
        except (ValueError, AttributeError):
            coord_x = None
        
        try:
            if record.coord_y and record.coord_y.strip():
                coord_y = float(record.coord_y.strip())
        except (ValueError, AttributeError):
            coord_y = None
        ...
        direccion = Direccion.get_or_create_by_location(
            ...
            parroquia='',  # No disponible en mecanización
            recinto=(record.recinto or '').strip(),
            coord_x=coord_x,
            coord_y=coord_y
        )
    ...
    def _get_or_create_beneficiario(self, session: Session, record: StgMecanizacion, 
                                   direccion: Direccion, asociacion: Asociacion = None) -> Beneficiario:
        """Crea o obtiene un beneficiario."""
        nombres_completos = (record.nombres_apellidos or '').strip() or 'NO_ESPECIFICADO'
