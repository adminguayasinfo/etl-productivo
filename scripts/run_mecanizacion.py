#!/usr/bin/env python3
"""
Script para ejecutar el pipeline de mecanización.
Carga datos desde stg_mecanizacion a la estructura operational usando el pipeline MecanizacionOperationalPipeline.
"""
...
from src.pipelines.operational_refactored.mecanizacion_operational_refactored_pipeline import MecanizacionOperationalRefactorizedPipeline


# Configurar logger
log_dir = Path("logs")
log_dir.mkdir(exist_ok=True)
log_file = log_dir / f"mecanizacion_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def run_pipeline(batch_size: int, dry_run: bool):
    """Ejecutar pipeline de mecanización."""
    
    logger.info("=== INICIANDO PIPELINE DE MECANIZACIÓN ===")
    ...
    try:
        ...
        pipeline = MecanizacionOperationalRefactorizedPipeline(batch_size=batch_size)
        result = pipeline.run()
...
def verify_staging_data():
    ...
    queries = [
        ("Total mecanización", 'SELECT COUNT(*) FROM "etl-productivo".stg_mecanizacion'),
        ("Procesados", 'SELECT COUNT(*) FROM "etl-productivo".stg_mecanizacion WHERE processed = true'),
        ("Pendientes", 'SELECT COUNT(*) FROM "etl-productivo".stg_mecanizacion WHERE processed = false'),
        ("Con errores", 'SELECT COUNT(*) FROM "etl-productivo".stg_mecanizacion WHERE error_message IS NOT NULL')
    ]
    ...
    try:
        result = db_connection.execute_query(
            'SELECT cultivo, COUNT(*) FROM "etl-productivo".stg_mecanizacion '
...
def verify_operational_data():
    ...
    queries = [
        ...
        ("Beneficios mecanización", 'SELECT COUNT(*) FROM "etl-productivo".beneficio_mecanizacion'),
    ]
    ...
    try:
        result = db_connection.execute_query(
            ...
            'WHERE b.tipo_beneficio = \'MECANIZACION\' '
            'GROUP BY tc.nombre ORDER BY COUNT(*) DESC'
        )
        if result:
            logger.info("  Distribución por cultivo (beneficios mecanización):")
            for cultivo, count in result:
                logger.info(f"    - {cultivo}: {count:,}")
    except Exception as e:
        logger.warning(f"  Error consultando distribución por cultivo: {e}")
    ...
    try:
        result = db_connection.execute_query(
            ...
            'JOIN "etl-productivo".tipo_cultivo tc ON b.tipo_cultivo_id = tc.id '
            'WHERE b.tipo_beneficio = \'MECANIZACION\' AND b.hectareas_beneficiadas IS NOT NULL '
            'GROUP BY tc.nombre ORDER BY total_ha DESC'
        )
        if result:
            logger.info("  Hectáreas por cultivo (beneficios mecanización):")
